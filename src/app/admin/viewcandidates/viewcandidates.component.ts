...
export class ViewcandidatesComponent implements OnInit{
  jobPositions: any[] = [];
  constructor(private jobListingService : JoblistingService,private applicantService: ApplicantService) { }
  ...
  onOptionSelected(){
    if(this.selectedJobId>0){
      this.applicantService.getApplicantsByJobId(this.selectedJobId).subscribe(data => {
